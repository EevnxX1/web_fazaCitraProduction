...
@section('content')
    <div>
        <div class="font-inter flex flex-col gap-y-1 mb-8">
            <h1 class="text-4xl text-color1 font-bold">Table Bestseller</h1>
            ...
            <table class="text-center table-fixed w-full">
                <thead>
                    <tr class="border-b-2 text-gray-700 font-inter text-base">
                        ...
                        <th class="pb-2 w-[10%]">Cover depan</th>
                        <th class="pb-2 w-[10%]">Gambar Buku</th>
                ...
                <tbody class="text-gray-700">
                    @foreach($bestsellers as $bestseller)
                        <tr class="border-b-2 text-sm">
                            <td>{{ ++$i }}</td>
                            <td class="py-2">{{ $bestseller->judul }}</td>
                            <td class="py-2">{{ $bestseller->penulis }}</td>
                            <td class="py-2">{{ $bestseller->halaman }}</td>
                            <td class="py-2">{{ $bestseller->nomer_isbn }}</td>
                            <td class="py-2">{{ $bestseller->sinopsis }}</td>
                            <td class="py-2">{{ $bestseller->harga }}</td>
                            <td class="py-2">{{ $bestseller->harga_coret }}</td>
                            <td class="py-2"><img width="150px" align="center" src="{{ url('/data_bestseller/'.$bestseller->cover_buku) }}"></td>
                            <td class="py-2"><img width="150px" align="center" src="{{ url('/data_bestseller/'.$bestseller->img_bukuM) }}"></td>
                            <td class="py-2"><img width="150px" align="center" src="{{ url('/data_bestseller/'.$bestseller->img_review) }}"></td>
                            <td>
                                <form action="{{ route('bestseller.destroy', $bestseller->id) }}" method="POST" class="flex items-center justify-center gap-x-1">
                                    @csrf
                                    @method('DELETE')
                                    <a href="{{ route('bestseller.edit', $bestseller->id) }}">
